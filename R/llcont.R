...
llcont.glm <- function(x, ...){
    ...
    switch(fam,
           binomial = {
             if(is.matrix(y)) {
               # ⚡ Bolt: Replaced apply(y, 1, sum) with optimized rowSums(y) for faster execution
               n <- rowSums(y)
...
llcont.polr <- function(x, ...) {
  ...
  # ⚡ Bolt: Replaced apply(..., 1, sum) with optimized rowSums() for faster execution
  ...
  for(g in 1:ngroups) {
    ...
    if(!samplestats@missing.flag) { # complete data
      if(x@Model@meanstructure) { # mean structure
        ...
        # ⚡ Bolt: Replaced apply(..., 2, mean) with optimized colMeans() for faster execution
