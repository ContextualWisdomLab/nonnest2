    ...
    switch(fam,
           binomial = {
             if(is.matrix(y)) {
               ...
               y_new <- n * 0
               if (any(n > 0)) y_new[n > 0] <- y[n > 0, 1] / n[n > 0]
               y <- y_new
             } else {
             ...
             ## Bolt: replaced ifelse() with preallocation and vectorized subsetting for performance
             wt_new <- m * 0
             wt_sub <- if (length(wt) == 1) wt else wt[m > 0]
             if (any(m > 0)) wt_new[m > 0] <- (wt_sub / m[m > 0])
             wt <- wt_new
...
llcont.hurdle <- function(x, ...) {
  ...
  zeroPoisson <- function(parms) {
    ...
    if (any(Y1)) res[Y1] <- w_sub * log(1 - exp(loglik0[Y1]))
  ...
  countNegBin <- function(parms) {
    ...
    res <- Y * 0
    w_sub <- if (length(weights) == 1) weights else weights[Y1]
    if (any(Y1)) res[Y1] <- w_sub * loglik1[Y1] - w_sub * log(1 - exp(loglik0[Y1]))
    res
    }
